...
- Include at least two unique useful reader-facing root-relative internal destinations when the packet provides them. If only one unique verified existing route is supplied, use it once; never invent or duplicate a destination, alias the same page, or link a planned sibling as live. Markdown image URLs do not count as internal links.
